...
Specialist personas adapted from [@chankov/agent-skills](https://github.com/chankov/agent-skills) v0.4.2 (MIT), with local customizations — imported from the live installed copies, not upstream. Upstream has since moved to [agent-fleet](https://github.com/chankov/agent-fleet). The pm kernel, charter format, and researcher/oracle/worker personas are original to this repo. MIT — see LICENSE.
